...
- 启动后台检查更新，有新版时居中提示；内置 aria2，支持断点续传、圆环进度、大小/速度/剩余时间。点击“下载并更新”后，校验摘要与发布者签名，自动在原目录替换、使用新版文件名并重启，保留旧 EXE 备份。可选自动下载仅提前下载，仍需确认本次更新后才替换。
...
| **0.3.1 · github-9.15** | D3D12 / SM75 / SM86 | 6X，取决于游戏 | 六种入口可多选；不包含 310.1 备选版 |
| 初始方案 · GitHub 第一版 | 沿用原 R2/SM86 能力 | 取决于原方案和游戏 | 仅显示随该方案提供的入口 |

本项目 0.2.6 的五种入口为 `version.dll`、`winmm.dll`、`dinput8.dll`、`winhttp.dll`、`dxgi.dll`，建议每次仅选一个。多选可能产生冲突，不代表兼容性更好。每个代理已集成桥接，无需外接 `rtxfg_vk_bridge.dll`。RTX20 使用 SM75 路径，RTX30 使用 SM86 路径。

6X 是可请求的倍率上限，**不会自动添加游戏菜单**。例如永劫无间菜单最高 4X，选择实验方案也不会出现 5X/6X。

上游 0.3.1 使用 310.9 运行库，修复了上游 0.3.0 在 RTX20/Turing 上无法启用帧生成的问题，同时支持 SM75 与 SM86；它仍仅支持 D3D12，不包含本项目的 Vulkan 修复。六种入口为 `version.dll`、`winmm.dll`、`dinput8.dll`、`dbghelp.dll`、`dxgi.dll`、`d3d12.dll`，可同时勾选，游戏先加载的一个生效，其余只转发。云端 DLL 已由本项目发布者重新数字签名。

选择单个游戏后，可在补丁设置中独立调整优化模式、倍率上限、UI 重组预设和日志级别。参数按游戏记忆；已部署游戏完全退出后再次点击“安装补丁”，软件只更新这些参数并保留 INI 中其他高级键。参数旁的小问号提供简短说明。
