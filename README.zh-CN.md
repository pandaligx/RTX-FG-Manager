...
管理器不再内置游戏 DLL，首次安装按所选方案和入口下载对应压缩包。默认使用国内 HTTPS 文件服务器，失败后尝试 GitHub；验证通过的缓存可离线复用。下载完成并校验后才写入游戏目录，卸载不依赖网络。
...
| **0.3.0 · github-9.14** | 仅 D3D12 / SM86 | 6X，取决于游戏 | 六种入口，必须单选；不包含 310.1 备选版 |
...
上游 0.3.0 使用 310.9 运行库，保留原件及其自签名；未合入本项目的 SM75 / Vulkan / 游戏专项修复，不作为 RTX20 或 Vulkan 方案。其入口为 `version.dll`、`winmm.dll`、`dbghelp.dll`、`dinput8.dll`、`dxgi.dll`、`d3d12.dll`。本次没有修改任何 DLL。
