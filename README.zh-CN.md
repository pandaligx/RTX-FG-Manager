...
- 默认 **0.2.5 · DX12/Vulkan（正式）** 和 `version.dll`，保留“初始方案 · GitHub 第一版”（R2/SM86）。Native 支持 `version.dll`、`winmm.dll`、`dinput8.dll`、`winhttp.dll`、`dxgi.dll` 五种入口，每次建议选择一个。
- 可选 **0.2.4 · 5X/6X · DX12/Vulkan（实验）**。6X 是倍率上限，实际倍率由游戏请求，不会自动增加游戏菜单；具体验证范围见 Release 说明。
- 五语言使用说明与硬件加速 GPU 计划检测；确认未开启时弹窗提醒，已开启不提示，可直接打开 Windows 图形设置。
...
基于 `dlssg_for_sm86`，由小南瓜维护以下扩展与修复：

- **兼容修复**：保留 RTX20 的 R2 适配；Native Fix1 修复无类型纹理的视图格式处理，解决已复现的永劫无间开启帧生成后进对局崩溃问题。
- **新增 Vulkan**：接入 Vulkan 资源与 DX12 推理后端，补充资源互操作和同步；五种代理均集成为单 DLL，无需额外桥接 DLL。
- **传输与调度优化**：使用 GPU 共享传输、资源缓存复用与合并提交，减少 CPU 图像中转、重复分配及等待；不满足共享条件时保留回退路径。
- **实验倍率扩展**：在社区 5X/6X 扩展基础上合入 Fix1 和 Vulkan；游戏需主动请求对应倍率，永劫无间菜单最高仍为 4X。

这里的 **0.2.5 是本项目的扩展方案版本**，不是上游发行版本；沿用 310.1 模型与原生推理内核。实际效果取决于游戏、显卡和驱动，不承诺固定帧率或所有游戏兼容。

## 简单使用

先按软件提示检查 Windows 10/11 的“硬件加速 GPU 计划”，未开启时通过“打开图形设置”开启；提示需要重启时，重启后再测试帧生成。
