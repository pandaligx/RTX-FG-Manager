...
| 方案 | 图形接口 | 参数与入口 |
| --- | --- | --- |
| **0.3.5 · Github-9.20（默认）** | D3D12，SM75/SM86 | 六入口可多选；默认 4X，按游戏支持可设最高 6X |
...
上游六入口是 `version.dll`、`winmm.dll`、`dinput8.dll`、`dbghelp.dll`、`dxgi.dll`、`d3d12.dll`。前四种优先，后两种按需使用；多个入口中只有先加载的一个工作，其余转发。它不包含本项目的 Vulkan 扩展。0.2.6 五入口为 `version.dll`、`winmm.dll`、`dinput8.dll`、`winhttp.dll`、`dxgi.dll`，建议每次选择一个，无需额外桥接 DLL。

右侧 **预设参数** 折叠面板按方案显示选项，点 **?** 查看说明。上游提供内核档位、倍率上限、UI 重组及日志；0.2.6 提供倍率、采样与日志；初始方案提供启用、倍率与日志。每游戏、每方案独立记忆，保持默认即可。完全退出游戏后点安装应用；相同 DLL 只更新受管参数，保留其他 INI 内容。**倍率上限不会添加游戏菜单，也不保证帧率按倍数增长。** 0.2.6 Vulkan 桥接日志暂不受日志级别控制。
